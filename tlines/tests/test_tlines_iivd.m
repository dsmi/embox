function test_tlines_iivd
% Test of calc_iivd function - use calc_ivd and do the numerical integration
% of the current
...
for iobs=1:5
	...
	zi=linspace(tl.z(iobs)+dl/2, tl.z(iobs+1)-dl/2, n);
        lm = (zi-tl.z(iobs))*a + b;
	test_ii = sum(calc_ivd(tl, zi, iobs, jsrc).*lm)*dl;
	ii      = calc_iivd(tl, iobs, a, b, 1, jsrc);
	tol = 1e-100+abs(ii)*1e-10;
