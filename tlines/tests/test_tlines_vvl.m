function test_tlines_vvl
% Test of calc_vvl function
...
test_v3 = sum(calc_vv(tl, tl_z(3)+3e-4, 3, zi, 3).*lm)*dl;
v3 = calc_vvl(tl, tl_z(3)+3e-4, 3, 3, l);
assertEquals(test_v3,v3,1e-14);

test_v2 = sum(calc_vv(tl, tl_z(2)+1e-3, 2, zi, 3).*lm)*dl;
v2 = calc_vvl(tl, tl_z(2)+1e-3, 2, 3, l);
assertEquals(test_v2,v2,1e-14);

test_v1 = sum(calc_vv(tl, tl_z(1)+3e-4, 1, zi, 3).*lm)*dl;
v1 = calc_vvl(tl, tl_z(1)+3e-4, 1, 3, l);
assertEquals(test_v1, v1, 1e-14);

test_v4 = sum(calc_vv(tl, tl_z(4)+5e-4, 4, zi, 3).*lm)*dl;
v4 = calc_vvl(tl, tl_z(4)+5e-4, 4, 3, l);
assertEquals(test_v4,v4,1e-14);

test_v5 = sum(calc_vv(tl, tl_z(5)+2e-3, 5, zi, 3).*lm)*dl;
v5 = calc_vvl(tl, tl_z(5)+2e-3, 5, 3, l);
...
test_v5v1 = sum(calc_vv(tl, tl_z(5)+2e-3, 5, zi, 1).*lm)*dl;
v5v1 = calc_vvl(tl, tl_z(5)+2e-3, 5, 1, l);
